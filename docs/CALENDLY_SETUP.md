...
- **Console messages from the Calendly embed (otBannerSdk, about:blank sandbox, Facebook/Stripe blocked, font CSP):** These come from **Calendly’s own scripts** inside the iframe (OneTrust consent banner, tracking, etc.). Bailadmin does not control that code; the embed still works. You can ignore “Blocked script execution in 'about:blank'”, “ERR_BLOCKED_BY_CLIENT” for facebook.net, and font/style CSP violations from `m.stripe.network` or `GdocApi`—they are from third-party frames. To reduce impact on your app’s perceived speed, the Calendly iframe is lazy-loaded (src set after the first paint).
