-- Buying a new package must not resurrect expired rows in active_packs (JSON).
-- Previously activate_package_for_student did: old_packs || new_pack with all history,
-- so UI summed stale pack counts + new pack (e.g. 3 + 8 = 11) while balance could stay 0+8.
...
'Activate package. Drops expired active_packs rows before appending new pack. plan_limit for Aure; respects subscription expiry_date.';
