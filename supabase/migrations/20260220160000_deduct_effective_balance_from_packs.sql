-- Allow deduct_student_classes to accept deduction when balance is in active_packs
-- (e.g. balance_events 0 but event_count in packs). Use effective balance for private/event.
...
COMMENT ON FUNCTION public.deduct_student_classes(text, uuid, int, text) IS 'Deduct group, private, or event. Uses effective balance from active_packs for private/event.';
