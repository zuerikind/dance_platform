...
BEGIN
  ...
  -- Effective balance (no deduction at registration)
  ...
  ELSIF v_student.balance IS NOT NULL THEN
    ...
    IF v_student.active_packs IS NOT NULL AND jsonb_array_length(v_student.active_packs) > 0 THEN
      ...
      END LOOP;
    END IF;
    -- Available = balance minus classes already registered for (future, not deducted)
    ...
    IF v_available IS NOT NULL AND v_available < 1 THEN
      RAISE EXCEPTION 'You don''t have enough classes in your package. You have % left and are already registered for % classes, so you only have % classes left.', v_effective_balance, v_registered_count, GREATEST(0, v_effective_balance - v_registered_count);
    END IF;
  END IF;
...
BEGIN
  ...
  IF v_student.balance IS NOT NULL THEN
    ...
    IF v_student.active_packs IS NOT NULL THEN
      ...
      END LOOP;
    END IF;
    -- Count already registered for (future, not deducted)
    SELECT count(*) INTO v_registered_count
    FROM public.class_registrations
    WHERE student_id = p_student_id
      AND school_id = p_school_id
      ...
      AND deducted = false;
    v_available := v_effective_balance - v_registered_count;
    -- How many of p_dates is the student not already registered for?
    ...
    IF v_available IS NOT NULL AND v_need_new > 0 AND v_available < v_need_new THEN
      RAISE EXCEPTION 'You don''t have enough classes in your package to sign up for % more classes. You have % left and are already registered for % classes, so you only have % classes left.', v_need_new, v_effective_balance, v_registered_count, GREATEST(0, v_effective_balance - v_registered_count);
    END IF;
  END IF;
