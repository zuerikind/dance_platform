-- =============================================================================
-- BUGFIX: activating an "unlimited" package (group+private+event limits all 0)
-- silently credited nothing.
--
-- In activate_package_for_student, a non-private-teacher plan with all limits 0
-- sets v_is_unlimited_group := true (intending to grant an unlimited group pack),
-- but the next guard `IF v_limit_group <= 0 AND v_limit_private <= 0 AND
-- v_limit_events <= 0 THEN RETURN` fired first and aborted — so the unlimited
-- pack was never created, balance/active_packs were never touched, and paid was
-- never set to true. Net effect: an approved "0 = unlimited" package left the
-- student with 0 classes and paid = false (unable to register).
--
-- Fix: only take the early RETURN when the plan is genuinely empty, i.e. NOT the
-- unlimited case. Body is otherwise copied verbatim from 20260502150000.
-- =============================================================================
...
BEGIN
  ...
  -- Only bail when the plan grants genuinely nothing. An unlimited plan (all limits 0,
  -- non-private-teacher) must proceed so the unlimited pack below is actually created.
  IF NOT v_is_unlimited_group
     AND v_limit_group <= 0 AND v_limit_private <= 0 AND v_limit_events <= 0 THEN
    RETURN;
  END IF;
...
'Activate package: drop expired packs; set group/private/event balances from remaining pack JSON plus this purchase. All-limits-zero (non-private-teacher) grants an UNLIMITED group pack (balance NULL) instead of silently no-opping.';
