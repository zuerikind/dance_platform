-- Fix effective class counting for Aure registration flows.
-- Root issue: some RPCs used balance + sum(active_packs.count), which can overcount.
-- Correct model (used elsewhere): effective_group_balance = max(balance, sum(non-expired pack counts)),
-- with unlimited if balance is NULL or any non-expired pack count is NULL.
