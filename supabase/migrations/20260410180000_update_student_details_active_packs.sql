-- Allow admin save to persist active_packs together with balances so effective balance
-- (max(students.balance, sum(pack counts))) matches manual corrections after package removal.
...
COMMENT ON FUNCTION public.update_student_details(text, uuid, text, text, text, text, numeric, timestamptz, int, int, jsonb) IS 'Update student details; optional p_active_packs keeps balances and pack rows aligned; returns updated row as jsonb.';
