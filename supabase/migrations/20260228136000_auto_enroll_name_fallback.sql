-- When auto_enroll_student creates a new enrollment and the user has no profiles row and is not an admin,
-- v_name stays empty and we insert NULL into both students and student_profiles → "Alumno Desconocido".
-- Add final fallback: use auth email or a placeholder so we never create completely nameless students.
...
BEGIN
  ...
  -- Final fallback: user has no profiles row and is not admin. Use auth email or placeholder.
  IF v_name IS NULL OR trim(v_name) = '' THEN
    v_auth_email := nullif(trim((auth.jwt()->>'email')::text), '');
    v_name := COALESCE(v_auth_email, 'Usuario (' || left(p_user_id::text, 8) || ')');
    v_email := COALESCE(nullif(trim(v_email), ''), v_auth_email);
  END IF;
...
COMMENT ON FUNCTION public.auto_enroll_student(uuid, uuid) IS 'Create enrollment for school; syncs profiles->student_profiles. Uses admin name/email/phone when profile is empty. Final fallback: auth email or placeholder to avoid "Alumno Desconocido".';
