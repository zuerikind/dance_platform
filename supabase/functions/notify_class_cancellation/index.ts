// Email school admin(s) when an Aure student cancels a class registration/request.
// Caller must be the same student (JWT user_id matches registration's student).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (reg.status !== 'cancelled') {
      return new Response(JSON.stringify({ ok: false, message: `Registration status is ${reg.status}` }), {
        status: 200,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    const html = buildAureEmailLayout({
      logoUrl,
      schoolName,
      preheader: `Cancelación de clase suelta — ${studentName}`,
      headline: 'Clase suelta cancelada',
      greetingLine: 'Hola,',
      bodyHtml: [
        `<strong>${escapeHtml(studentName)}</strong> canceló su registro/solicitud en <strong>${escapeHtml(schoolName)}</strong>.`,
      ],
      details: [
      ...
      closingLine: 'El cupo ha quedado disponible nuevamente.',
      signOff: schoolName,
    });

    const subject = `Clase suelta cancelada — ${studentName} — ${dateStr}`;
    ...
    return new Response(JSON.stringify({ ok: true, message: 'Admin notified of cancellation' }), {
      status: 200,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (e) {
