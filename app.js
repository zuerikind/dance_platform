(() => {
  ...
  function parseHashRoute() {
    ...
    if (segments[0] === "settings") {
      ...
      if (params.get("calendly") === "error") {
        ...
        fetch("http://127.0.0.1:7243/ingest/adf50a45-9f8f-4c1e-8e97-90df72d1c8da", { method: "POST", headers: { "Content-Type": "application/json", "X-Debug-Session-Id": "9cbdaf" }, body: JSON.stringify({ sessionId: "9cbdaf", location: "routing.js:parseHashRoute(calendly=error)", message: "Calendly callback error", data: { message: msg, errcode, school_id: schoolId || null }, timestamp: Date.now(), hypothesisId: "B" }) }).catch(() => {
        });
  ...
  var DANCE_LOCALES = {
    en: {
      ...
      cancel_this_class: "Cancel this class",
      cancel_all_n_classes: "Cancel all {n} classes",
      cancel_all_confirm_n: "Cancel your registration for all {n} remaining dates?",
    ...
    es: {
      ...
      cancel_this_class: "Cancelar esta clase",
      cancel_all_n_classes: "Cancelar las {n} clases",
      cancel_all_confirm_n: "\xBFCancelar tu registro para las {n} fechas restantes?",
      ...
      export_calendar_ics_hint: "A\xF1adir a Google Calendar, Apple Calendar o cualquier app .ics.",
      check_in_btn: "Registrar",
      mark_no_show_btn: "Marcar ausencia",
      todays_private_lessons: "Clases privadas de hoy",
      checked_in: "Registrado",
      private_lesson_cancellation_policy: "Si cancelas con menos de 4 horas de antelaci\xF3n, se descontar\xE1 un cr\xE9dito. Si no asistes y el profesor no te registra, tambi\xE9n perder\xE1s un cr\xE9dito.",
      cancel_late_warning: "Cancelar ahora usar\xE1 un cr\xE9dito de clase privada.",
      cancel_btn: "Cancelar",
      ...
      profile_your_avg_rating: "Tu valoraci\xF3n media",
      ...
      calendly_sign_in_popup: "Inicia sesi\xF3n en Calendly en la ventana que se abri\xF3.",
      ...
      calendly_student_booking_mode: "Qu\xE9 ven los alumnos al reservar",
      calendly_mode_calendly: "Calendly",
    ...
    de: {
      ...
      cancel_this_class: "Diesen Kurs stornieren",
      cancel_all_n_classes: "Alle {n} Kurse stornieren",
      cancel_all_confirm_n: "Anmeldung f\xFCr alle {n} restlichen Termine stornieren?",
      ...
      export_calendar_ics_hint: "Zu Google Kalender, Apple Kalender oder einer .ics-App hinzuf\xFCgen.",
      check_in_btn: "Einchecken",
      mark_no_show_btn: "Absage markieren",
      todays_private_lessons: "Heutige Privatstunden",
      checked_in: "Eingecheckt",
      private_lesson_cancellation_policy: "Bei Absage weniger als 4 Stunden vor der Stunde wird ein Privatguthaben abgezogen. Wenn du nicht erscheinst und der Lehrer dich nicht eincheckt, verf\xE4llt ebenfalls ein Guthaben.",
      ...
      calendly_student_booking_mode: "Was Sch\xFCler bei der Buchung sehen",
      calendly_mode_calendly: "Calendly",
  ...
  var TEACHER_TIMEZONE_OPTIONS = [
    ...
    { value: "America/Bogota", label: "America/Bogot\xE1 (Medell\xEDn, Bogot\xE1 \u2014 Colombia Time, UTC-5)" },
    { value: "America/Sao_Paulo", label: "America/S\xE3o Paulo" },
  ...
  window.fetchProfileMyReviews = async () => {
    ...
    try {
      ...
      const avgLine = myAvg != null ? `<p style="font-size: 0.85rem; color: var(--text-secondary); margin-bottom: 0.75rem;">${(t2("profile_your_avg_rating") || "Your average rating").replace(/</g, "&lt;")}: <strong style="color: var(--text-primary);">${myAvg} \u2605</strong></p>` : "";
      const profileCategoryStarsHtml = (r) => {
        ...
        const rows = catKeys.map(({ key, val, label }) => {
          ...
          return `<div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 4px;"><span style="font-size: 0.85rem; color: var(--text-secondary);">${t2(label) || key}</span><span>${starHtml}</span></div>`;
        }).filter(Boolean);
      ...
      el.innerHTML = avgLine + `<ul class="profile-my-reviews-list" style="list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 0.75rem;">${reviews.map((r) => {
        ...
        return `<li><details class="profile-review-details" style="border: 1px solid var(--border); border-radius: 12px; background: var(--surface); overflow: hidden;">
                ...
                <div style="padding: 0 14px 12px; border-top: 1px solid var(--border);">
                    ...
                    ${r.target_id ? `<button type="button" class="btn-secondary" style="margin-top: 10px; padding: 8px 12px; font-size: 12px; border-radius: 10px; cursor: pointer;" onclick="${goJs}">${t2("discovery_go_to_reviews") || "View school"}</button>` : ""}
                </div>
            </details></li>`;
      }).join("")}</ul>`;
      if (window.lucide) window.lucide.createIcons();
      ...
      (async () => {
        ...
        const categoryStarsHtml = (r) => {
          ...
          const rows = catKeys.map(({ key, val, label }) => {
            ...
            return `<div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 4px;"><span style="font-size: 0.85rem; color: var(--text-secondary);">${t2(label) || key}</span><span>${stars}</span></div>`;
          }).filter(Boolean);
          if (!rows.length) return "";
          return `<div class="review-categories-inline" style="margin-top: 10px; padding-top: 10px; border-top: 1px solid var(--border);"><div style="font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; color: var(--text-secondary); margin-bottom: 6px;">${(t2("discovery_review_show_categories") || "Categories").replace(/</g, "&lt;")}</div>${rows.join("")}</div>`;
        };
        ...
        const schoolAvg = (reviews2) => {
          if (!reviews2.length) return null;
          const sum = reviews2.reduce((a, r) => a + (r.rating_overall != null ? Number(r.rating_overall) : 0), 0);
        ...
        if (needsReview.length > 0) {
          ...
          Object.keys(needsReviewBySchool).forEach((key) => {
            ...
            const avgHtml = avg != null ? `<span style="font-size: 0.85rem; font-weight: 600;">${avg} \u2605</span><span style="font-size: 0.85rem; font-weight: 600; color: var(--text-secondary);"> \xB7 ${schoolReviews.length}</span>` : `<span style="font-size: 0.85rem; font-weight: 600; color: var(--text-secondary);">${schoolReviews.length}</span>`;
      ...
      if (view === "school-selection") {
        ...
        if (!state.currentSchool?.id || teacherBookingSchool?.profile_type !== "private_teacher") {
          ...
          const studentListRest = state.loading ? "" : myClasses.length === 0 ? `
            ...
            const dateLabel = isLesson ? window.formatShortDate ? window.formatShortDate(new Date(item.start_at_utc), state.language) : new Date(item.start_at_utc).toLocaleDateString() : window.formatShortDate ? window.formatShortDate(/* @__PURE__ */ new Date(item.requested_date + "T00:00:00"), state.language) : item.requested_date;
        ...
        html += `
          ...
          const weekStarts = [...new Set(upcoming.map((r) => window.getWeekStartDateStr(r.class_date)))].filter(Boolean).sort();
          const weekIndex = Math.min(state.myRegistrationsCarouselWeekIndex || 0, Math.max(0, weekStarts.length - 1));
          const currentWeekStart = weekStarts[weekIndex] || weekStarts[0];
          const upcomingThisWeek = currentWeekStart ? upcoming.filter((r) => window.getWeekStartDateStr(r.class_date) === currentWeekStart).sort((a, b) => (a.class_date + (a.time || "")).localeCompare(b.class_date + (b.time || ""))) : [];
          const weekStartDate = currentWeekStart ? /* @__PURE__ */ new Date(currentWeekStart + "T12:00:00") : null;
          ...
          return `
                    <div class="qr-registrations-expandable ${regExpanded ? "expanded" : ""}" style="margin-top: 1.5rem; width: 100%; max-width: 320px; margin-left: auto; margin-right: auto; border-top: 1px solid var(--border); padding-top: 1rem;">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 14px;">
                            <div>
                                ${upcoming.length === 0 ? `<div style="font-size: 10px; font-weight: 700; text-transform: uppercase; color: var(--text-secondary); opacity: 0.7; margin-bottom: 8px;">${t2.upcoming_classes || "Upcoming"}</div><div style="font-size: 13px; color: var(--text-secondary); padding: 8px 0;">${t2.no_upcoming || "No upcoming registrations"}</div>` : `
                                <div class="qr-reg-carousel-nav" style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px;">
                                    <button type="button" class="btn-ghost" style="padding: 4px 8px; font-size: 18px;" ${weekIndex <= 0 ? "disabled" : ""} onclick="state.myRegistrationsCarouselWeekIndex=${(state.myRegistrationsCarouselWeekIndex || 0) - 1}; renderView();"><i data-lucide="chevron-left" size="20"></i></button>
                                    <span style="font-size: 10px; font-weight: 700; text-transform: uppercase; color: var(--text-secondary);">${weekLabel}</span>
                                    <button type="button" class="btn-ghost" style="padding: 4px 8px; font-size: 18px;" ${weekIndex >= weekStarts.length - 1 ? "disabled" : ""} onclick="state.myRegistrationsCarouselWeekIndex=${(state.myRegistrationsCarouselWeekIndex || 0) + 1}; renderView();"><i data-lucide="chevron-right" size="20"></i></button>
                                </div>
                                ${upcomingThisWeek.length === 0 ? `<div style="font-size: 13px; color: var(--text-secondary); padding: 8px 0;">${t2.no_upcoming || "No upcoming registrations"}</div>` : upcomingThisWeek.map((r) => {
            const dateLabel = fmtDate(r.class_date);
            const canCancel = canCancelReg(r);
            const sameClassUpcoming = upcoming.filter((x) => x.class_id === r.class_id);
          ...
          return `
                    <div class="teacher-accepted-classes-expandable ${expanded ? "expanded" : ""}" style="padding: 0 1.2rem; margin-bottom: 1rem;">
                        ...
                        <div id="teacher-accepted-classes-content" style="padding: 0.8rem 0; display: ${expanded ? "" : "none"};">
                            ...
                            <div style="margin-bottom: 12px; padding: 10px; background: var(--system-gray6); border-radius: 12px;">
                                <div style="font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: var(--text-secondary); margin-bottom: 8px;">${t2.todays_private_lessons || "Today's private lessons"}</div>
                                ${todaysLessons.map((l) => {
            ...
            return `
                                <div class="teacher-accepted-class-row" style="display: flex; align-items: center; gap: 12px; padding: 10px 12px; background: var(--system-gray6); border-radius: 12px; margin-bottom: 8px;">
                                    ...
                                    ${lessonId && isPast && lesson.status === "confirmed" && !lesson.credit_deducted ? `<button type="button" class="btn-secondary" style="padding: 6px 10px; font-size: 12px; flex-shrink: 0;" onclick="window.markPrivateLessonNoShow('` + lessonId + `')">` + (t2.mark_no_show_btn || "Mark no-show") + "</button>" : ""}
                                </div>`;
          }).join("")}
                            ` : (() => {
            ...
            <div class="ios-list" style="overflow: visible;">
                <div class="ios-list-item" style="flex-direction: column; align-items: stretch; gap: 6px; padding: 14px 16px;">
                    ...
                    <select id="teacher-availability-timezone" style="background: var(--system-gray6); border: none; border-radius: 10px; padding: 10px 12px; font-size: 14px; color: var(--text-primary); outline: none; width: 100%; cursor: pointer; -webkit-appearance: none; appearance: none; background-image: url('data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 width=%2224%22 height=%2224%22 viewBox=%220 0 24 24%22 fill=%22none%22 stroke=%22%23666%22 stroke-width=%222%22><polyline points=%226 9 12 15 18 9%22></polyline></svg>'); background-repeat: no-repeat; background-position: right 10px center; background-size: 16px; padding-right: 36px;">
                        ${(() => {
          const currentTz = state.teacherAvailabilitySettings?.timezone || "UTC";
          const opts = TEACHER_TIMEZONE_OPTIONS.some((o) => o.value === currentTz) ? TEACHER_TIMEZONE_OPTIONS : [{ value: currentTz, label: currentTz }, ...TEACHER_TIMEZONE_OPTIONS];
          return opts.map((o) => `<option value="${o.value.replace(/"/g, "&quot;")}" ${currentTz === o.value ? "selected" : ""}>${(o.label || o.value).replace(/</g, "&lt;")}</option>`).join("");
        })()}
                    </select>
                    ...
                    <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap; margin-top: 4px;">
                        <button type="button" id="teacher-timezone-save-btn" class="btn-secondary" onclick="window.saveTeacherTimezone()" style="padding: 8px 14px; font-size: 13px; display: inline-flex; align-items: center; gap: 6px;">${t2.save || "Save"}</button>
            ...
            <div class="ios-list" style="padding: 0 1.2rem 1rem;">
                ...
                <div class="ios-list-item" style="flex-direction: column; align-items: stretch; gap: 10px;">
                    ...
                    <p style="font-size: 13px; color: var(--text-secondary); margin: 0;">${t2.calendly_connect_desc || "Connect your Calendly account so students can book private classes from your live availability."}</p>
                    <div style="display: flex; flex-direction: column; gap: 8px;">
                        <button type="button" id="calendly-connect-btn" class="btn-primary" ${(state.adminSettings || {}).use_calendly_for_booking === "false" || state._calendlyOAuthLoading ? "disabled" : ""} onclick="(state.adminSettings || {}).use_calendly_for_booking !== 'false' && !state._calendlyOAuthLoading && window.startCalendlyOAuth()" style="border-radius: 12px; padding: 12px 16px; font-weight: 600; display: inline-flex; align-items: center; justify-content: center; gap: 8px; ${(state.adminSettings || {}).use_calendly_for_booking === "false" ? "opacity: 0.6; cursor: not-allowed;" : ""}">
                            ${state._calendlyOAuthLoading ? `<i data-lucide="loader-2" class="spin" size="18"></i> ${t2.calendly_connecting || "Connecting\u2026"}` : t2.calendly_connect_btn || "Connect Calendly"}
                        </button>
                        ${state._calendlyOAuthPopupOpen ? `<p style="font-size: 13px; color: var(--text-secondary); margin: 0;">${t2.calendly_sign_in_popup || "Sign in to Calendly in the separate window."}</p>` : ""}
                    </div>
                </div>
                `}
  ...
  window.getWeekStartDateStr = (dateStr) => {
    ...
    return window.formatClassDate(monday);
  };
  ...
  window.showCancelAllConfirmModal = (registrationIds) => {
    const t2 = typeof window.t === "function" ? window.t : (k) => k;
    const studentId = state.currentUser?.id;
    if (!studentId || !supabaseClient || !registrationIds || registrationIds.length === 0) return;
    const n = registrationIds.length;
    window.showMessageModal({
      icon: "warning",
      title: t2("cancel_registration"),
      body: (t2("cancel_all_confirm_n") || "Cancel your registration for all {n} remaining dates?").replace("{n}", n),
      primaryLabel: t2("cancel_confirm_yes"),
      secondaryLabel: t2("go_back"),
      onSecondary: (close) => {
        close();
  ...
  window.registerForClassMonthly = async (classId, className) => {
    ...
    try {
      ...
      window.showMessageModal({
        ...
        body: "",
        primaryLabel: t2("got_it"),
        secondaryLabel: t2("my_registrations_label") || "View my registrations",
        onSecondary: (close) => {
          close();
          state.currentView = "qr";
          state.qrRegistrationsExpanded = true;
...
School: ${schoolName}`)) return;
  ...
  window.saveTeacherTimezone = async () => {
    ...
    const t2 = DANCE_LOCALES[state.language || "en"];
    const saveLabel = t2.save || "Save";
    const savingLabel = t2.saving_label || "Saving\u2026";
    const savedLabel = t2.competition_saved_indicator || t2.profile_saved_success || "Saved";
  ...
  function buildIcsFromEvents(events) {
    ...
    const dtstamp = formatUtcForIcs(/* @__PURE__ */ new Date());
    ...
    events.forEach(function(ev) {
      ...
      lines.push("SUMMARY:" + (ev.summary || "Private lesson").replace(/[,;\\]/g, function(c) {
        return "\\" + c;
      }));
      if (ev.description) lines.push("DESCRIPTION:" + String(ev.description).replace(/[,;\\]/g, function(c) {
        return "\\" + c;
      }));
      if (ev.location) lines.push("LOCATION:" + String(ev.location).replace(/[,;\\]/g, function(c) {
        return "\\" + c;
      }));
  ...
  window.downloadCalendarIcsOne = function(lessonOrEventOrId, type) {
    ...
    if (!start || !end) {
      alert("Cannot export: missing date/time.");
      return;
    }
    let summary = lesson.summary;
    if (!summary && type === "teacher" && lesson.student_id) {
      const s = (state.students || []).find(function(x) {
        return String(x.id) === String(lesson.student_id);
      });
  ...
  window.downloadCalendarIcs = async function(type, useClientOnly) {
    ...
    const tryClientFallback = function() {
      const events = [];
      if (type === "student") {
        const lessons = (state.studentPrivateLessons || []).filter(function(l) {
          return (l.status === "confirmed" || l.status === "attended") && l.start_at_utc && l.end_at_utc;
        });
        ...
        const lessons = (state.privateLessons || []).filter(function(l) {
          return (l.status === "confirmed" || l.status === "attended") && l.start_at_utc && l.end_at_utc;
        });
      ...
      if (events.length === 0) {
        alert(t2.export_no_events || "No classes to export.");
        return;
      }
    ...
    if (!sess && supabaseClient) {
      try {
        sess = (await supabaseClient.auth.getSession()).data?.session;
      } catch (_) {
      }
    }
