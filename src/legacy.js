...
const DANCE_LOCALES = {
    ...
    es: {
        ...
        admin_password_confirm_delete: "Introduce tu contraseña de administrador para quitar este registro:",
        ...
        cancel_all_confirm_n: "¿Cancelar tu registro para las {n} fechas restantes?",
    ...
    de: {
        ...
        cancel_all_confirm_n: "Anmeldung für alle {n} restlichen Termine stornieren?",
...
function _renderViewImpl() {
    ...
    if (view === 'school-selection') {
            ...
            <div class="teacher-booking-container" style="padding: 1.2rem; padding-bottom: 6rem;">
                ...
                <div class="teacher-booking-card">
                    ...
                    <div class="calendly-embed-wrap" style="margin: 0 18px 18px; min-height: 630px; border-radius: 16px; overflow: hidden; background: var(--system-gray6,#f2f2f7);">
                        ${(() => { const u = (state.teacherCalendlySelectionForBooking || {}).scheduling_url || ''; const name = (state.currentUser && state.currentUser.name) || ''; const email = (state.currentUser && state.currentUser.email) || ''; const sep = u.indexOf('?') >= 0 ? '&' : '?'; const q = [name && ('name=' + encodeURIComponent(name)), email && ('email=' + encodeURIComponent(email))].filter(Boolean).join('&'); const iframeUrl = u ? (q ? u + sep + q : u) : ''; return iframeUrl ? `<iframe id="calendly-inline-iframe" data-src="${iframeUrl.replace(/"/g, '&quot;')}" style="width: 100%; height: 630px; border: none;" title="${(t2.calendly_book || 'Book a session').replace(/"/g, '&quot;')}"></iframe>` : `<div style="padding: 2rem; text-align: center; color: var(--text-secondary); font-size: 14px;">${(t2.calendly_loading || 'Loading...')}</div>`; })()}
                    </div>
                    ` : `
                    ${locations.length ? `
        ...
        // Week context: use schedule week when in weekly view (for carousel), else current week
            ...
            <div class="schedule-my-classes" style="margin-bottom: 1.2rem; border: 1px solid var(--border); border-radius: 16px; padding: 14px 16px; background: var(--system-gray6);">
                <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: ${upcomingRegs.length > 0 ? '10px' : '0'};">
                    <div style="display: flex; align-items: center; gap: 8px;">
                        ...
                        <span style="font-size: 13px; font-weight: 700; color: var(--text-primary);">${t.my_registrations_label || 'My Registered Classes'}</span>
                    ...
                    ${upcomingRegs.length > 0 ? `<button type="button" onclick="window.downloadCalendarIcs('student')" style="background: none; border: none; padding: 4px 8px; cursor: pointer; color: var(--text-secondary); display: flex; align-items: center; gap: 4px; font-size: 11px; font-weight: 600;" title="${(t.export_all_to_calendar || 'Export all').replace(/"/g, '&quot;')}"><i data-lucide="calendar-plus" size="14"></i></button>` : ''}
                </div>
                ${upcomingRegs.length === 0 ? `<div style="font-size: 12px; color: var(--text-secondary); padding: 4px 0;">${t.no_upcoming || 'No upcoming registrations'}</div>` :
                    ...
                    ${upcomingRegs.sort((a, b) => (a.class_date + (a.time || '')).localeCompare(b.class_date + (b.time || ''))).slice(0, 10).map(r => {
                        ...
                        return `<div style="display: flex; align-items: center; gap: 10px; padding: 8px 10px; background: var(--bg-body); border-radius: 12px;">
                            ...
                            <div style="display: flex; align-items: center; gap: 4px; flex-shrink: 0;">
                                <button type="button" onclick="window.downloadGroupClassIcsOne('${String(r.id || '').replace(/'/g, "\\'")}');" style="background: none; border: none; padding: 4px; cursor: pointer; color: var(--text-secondary);" title="${(t.export_to_calendar || 'Export').replace(/"/g, '&quot;')}"><i data-lucide="calendar-plus" size="14"></i></button>
                                ${canCancel ? `<button type="button" onclick="event.stopPropagation(); window.showCancelConfirmModal('${r.id}')" style="background: none; border: none; padding: 4px; cursor: pointer; color: var(--system-red);" title="${(t.cancel_registration || 'Cancel').replace(/"/g, '&quot;')}"><i data-lucide="x" size="14"></i></button>` : ''}
                            </div>
                        </div>`;
                    }).join('')}
                    ${upcomingRegs.length > 10 ? `<div style="font-size: 11px; color: var(--text-secondary); text-align: center; padding: 4px 0;">+ ${upcomingRegs.length - 10} ${t.more || 'more'}</div>` : ''}
                </div>`}
            </div>`;
        }
            ...
            <div class="text-center">
                ...
                return out;
                    ...
                    ${state.currentSchool?.class_registration_enabled ? (() => {
                        ...
                        const weekStartDate = currentWeekStart ? new Date(currentWeekStart + 'T12:00:00') : null;
                    ...
                    <div class="qr-registrations-expandable ${regExpanded ? 'expanded' : ''}" style="margin-top: 1.5rem; width: 100%; max-width: 320px; margin-left: auto; margin-right: auto; border-top: 1px solid var(--border); padding-top: 1rem;">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 14px;">
                            <div>
                                ${upcoming.length === 0 ? `<div style="font-size: 10px; font-weight: 700; text-transform: uppercase; color: var(--text-secondary); opacity: 0.7; margin-bottom: 8px;">${t.upcoming_classes || 'Upcoming'}</div><div style="font-size: 13px; color: var(--text-secondary); padding: 8px 0;">${t.no_upcoming || 'No upcoming registrations'}</div>` : `
                                ...
                                ${upcomingThisWeek.length === 0 ? `<div style="font-size: 13px; color: var(--text-secondary); padding: 8px 0;">${t.no_upcoming || 'No upcoming registrations'}</div>` : upcomingThisWeek.map(r => {
        ...
        html += `
            ...
            <div class="students-page">
                ...
                ${state.currentSchool?.class_registration_enabled ? (() => {
                    ...
                    // Only show occurrences that match the current schedule: class_date must fall on the class's scheduled weekday
                    const allGroupedArr = Object.values(grouped)
                        .filter(g => {
                            const cls = classes.find(c => c.id === g.class_id);
                            if (!cls) return false;
                            const scheduledDow = scheduleDayToDow(cls.day);
                            if (scheduledDow == null) return true;
                            const dateDow = new Date(g.class_date + 'T00:00:00').getDay();
...
// Uses mock date as "today" when state.mockDate is set (e.g. ?mockDate=2026-03-02), so "next" class is relative to mock.
...
// Returns {start: Date, end: Date} for the schedule weekly view; uses scheduleWeekOffset (0 = current week, ±1 = prev/next).
...
// Returns the date for a given day code within the schedule display week (for weekly view carousel).
...
// Returns YYYY-MM-DD of Monday of the week containing the given date string (for grouping registrations by week).
...
window.showCancelAllConfirmModal = (registrationIds) => {
    ...
    window.showMessageModal({
        icon: 'warning',
        title: t('cancel_registration'),
        body: (t('cancel_all_confirm_n') || 'Cancel your registration for all {n} remaining dates?').replace('{n}', n),
        primaryLabel: t('cancel_confirm_yes'),
        secondaryLabel: t('go_back'),
        onSecondary: (close) => { close(); },
        onPrimary: async (close) => {
...
window.registerForClassMonthly = async (classId, className) => {
    ...
    try {
        ...
        window.showMessageModal({
            ...
            body: '',
            primaryLabel: t('got_it'),
            secondaryLabel: t('my_registrations_label') || 'View my registrations',
            onSecondary: (close) => {
                close();
...
window.downloadGroupClassIcsOne = function(registrationId) {
    ...
    const reg = allRegs.find(function(r) { return String(r.id) === String(registrationId); });
    if (!reg || !reg.class_date) { alert('Cannot export: registration not found.'); return; }
    const cls = (state.classes || []).find(function(c) { return c.id === reg.class_id; });
    const startTime = reg.time || cls?.time || '00:00';
    const endTime = cls?.end_time || null;
    const start = new Date(reg.class_date + 'T' + startTime);
    let end;
    if (endTime) {
        end = new Date(reg.class_date + 'T' + endTime);
    } else {
...
window.downloadCalendarIcs = async function(type, useClientOnly) {
    ...
    const tryClientFallback = function() {
        ...
        if (type === 'student') {
            ...
            var groupRegs = (state.studentRegistrations || []).filter(function(r) { return r.status === 'registered' && r.class_date; });
            groupRegs.forEach(function(r) {
                var cls = (state.classes || []).find(function(c) { return c.id === r.class_id; });
                var startTime = r.time || (cls ? cls.time : null) || '00:00';
                var endTime = cls ? cls.end_time : null;
                var start = new Date(r.class_date + 'T' + startTime);
                var end = endTime ? new Date(r.class_date + 'T' + endTime) : new Date(start.getTime() + 60 * 60 * 1000);
                events.push({
